...
int run_stage_ffi(const fs::path& bin_dir, const std::string& stage, const fs::path& work_dir) {
    ...
    // Stage binaries are bundled as lib<stage>.so (see CMakeLists.txt's
    // ANDROID packaging block) since Gradle only ships real .so files in
    // the APK; extractNativeLibs="true" restores their execute bit at
    // install time. UNVERIFIED on-device — see CMakeLists.txt comment.
    fs::path exe = bin_dir / ("lib" + stage + ".so");
#else
    fs::path exe = bin_dir / stage;
#endif
    std::string cmd;
#ifdef __ANDROID__
    // The linker needs to find libonnxruntime.so and any other sibling
    // .so deps (e.g. for stage5_inference) — these live alongside the
    // stage binaries in bin_dir (Android's nativeLibraryDir), but a
    // plain exec()'d binary doesn't inherit Android's native-lib search
    // path automatically. Explicitly set LD_LIBRARY_PATH so the
    // dynamic linker looks there.
    cmd = "LD_LIBRARY_PATH=\"" + bin_dir.string() + "\" \"" + exe.string() + "\" \"" + work_dir.string() + "\"";
#else
    cmd = "\"" + exe.string() + "\" \"" + work_dir.string() + "\"";
#endif
#ifdef _WIN32
    cmd = "\"" + cmd + "\"";
#endif
