...
// center-proximity and Intersection-over-Minimum (IoM), and filters by area.
std::vector<Detection> deduplicate_boxes(std::vector<Detection> boxes, double center_dist_thresh) {
    if (boxes.empty()) return {};

    // Calculate median area to filter out tiny fragments or huge artifacts
    std::vector<double> areas;
    for (const auto& b : boxes) areas.push_back((b.x2 - b.x1) * (b.y2 - b.y1));
    std::sort(areas.begin(), areas.end());
    double median_area = areas[areas.size() / 2];

    // Sort by score descending so we keep the most confident boxes first
    ...
    for (const auto& b : boxes) {
        ...
        // Area must be within reasonable bounds of the median
        if (area_b < median_area * 0.4 || area_b > median_area * 2.5) continue;
        ...
        for (const auto& k : keep) {
            ...
            // 1. Center proximity check
            if (std::abs(cx - kcx) < center_dist_thresh && std::abs(cy - kcy) < center_dist_thresh) {
                is_dup = true;
                break;
            }
            
            // 2. Intersection-over-Minimum (IoM) check for overlaps
            double ix1 = std::max(b.x1, k.x1);
            double iy1 = std::max(b.y1, k.y1);
            double ix2 = std::min(b.x2, k.x2);
            double iy2 = std::min(b.y2, k.y2);
            if (ix2 > ix1 && iy2 > iy1) {
                double inter = (ix2 - ix1) * (iy2 - iy1);
                double area_k = (k.x2 - k.x1) * (k.y2 - k.y1);
                double min_area = std::min(area_b, area_k);
                // If a box is substantially overlapping another (e.g. fragment inside a bubble)
                if (inter / min_area > 0.50) {
                    is_dup = true;
                    break;
                }
            }
...
std::optional<json> process_inference(OnnxDetector& detector, const fs::path& stage4_path,
    ...
    // TUNING: annotation colors (BGR)
    // Confidence threshold below which a filled bubble is drawn as "review"
    constexpr double MODEL_CONF_DRAW_THRESH = 0.55;
    // Filled (confident)  — green
    const cv::Scalar COLOR_FILLED_TRUSTED(0, 200, 0);
    // Filled (low confidence, review needed) — yellow-green
    const cv::Scalar COLOR_FILLED_REVIEW(0, 200, 120);
    // Empty (cls 1) — red
    const cv::Scalar COLOR_EMPTY(0, 0, 220);
    // Canceled (cls 2) — blue
    const cv::Scalar COLOR_CANCELED(220, 80, 0);
    ...
    for (const auto& d : final_detections) {
        ...
        cv::Scalar color;
        bool is_canceled = (d.cls_id == 2);
        if (is_canceled) {
            color = COLOR_CANCELED;
        } else if (d.cls_id == 0) {
            color = (d.score >= MODEL_CONF_DRAW_THRESH) ? COLOR_FILLED_TRUSTED : COLOR_FILLED_REVIEW;
        } else {
            color = COLOR_EMPTY;  // cls 1 (empty / X'd out)
        }

        int x1i = static_cast<int>(d.x1), y1i = static_cast<int>(d.y1);
        int x2i = static_cast<int>(d.x2), y2i = static_cast<int>(d.y2);

        cv::rectangle(annotated_img, cv::Point(x1i, y1i), cv::Point(x2i, y2i), color, 2);

        char score_buf[16];
        std::snprintf(score_buf, sizeof(score_buf), "%.2f", d.score);
        cv::putText(annotated_img, score_buf, cv::Point(x1i, y1i - 5),
