    ...
    try {
        ...
        if (id_panel_path && std::strlen(id_panel_path) > 0) {
            ...
            if (!id_final.empty()) {
                ...
                auto [id_cols, id_str, id_letter] = questions::process_student_id(id_dets, config.id.num_digits, config.id.num_letters, config.id.letters, config.id.column_labels(), config.row_tolerance_px);
        ...
        if (mcq_panel_path && std::strlen(mcq_panel_path) > 0) {
            ...
            if (!mcq_final.empty()) {
                auto mcq_dets = inference::run_inference_mcq_adaptive(mcq_final, config.model_path, config.num_questions, config.num_question_columns, config.num_choices, 0.25f, 0.05f, 0.45f);
                final_result.questions = questions::process_questions(mcq_dets, config.num_questions, config.num_question_columns, config.choice_labels(), config.row_tolerance_px);
