...
class SessionManagerPage(QWidget):
    ...
    def _build_group_detail_ui(self):
        ...
        self.btn_set_password.setFont(QFont(self.fonts.orbitron, 12, QFont.Weight.Bold))
        ...
        self.btn_set_password.setStyleSheet(
            ...
            f"border: 2px solid {TRUE_AZURE}; border-radius: 8px; padding: 10px; margin-top: 10px; margin-bottom: 10px; }} "
    ...
    def open_session_password_dialog(self):
        if not self.project_manager.is_active:
            QMessageBox.warning(self, "Error", "No active workspace.")
            return
    ...
    def _build_monitoring_ui(self):
        ...
        proctors_lbl = QLabel("CONNECTED PROCTORS")
        proctors_lbl.setFont(QFont(self.fonts.orbitron, 9, QFont.Weight.Bold))
        proctors_lbl.setStyleSheet(f"color: {TEXT_MUTED}; letter-spacing: 1px;")
        content_layout.addWidget(proctors_lbl)

        self.proctors_list = QListWidget()
        self.proctors_list.setFixedHeight(110)
        self.proctors_list.setStyleSheet(
            f"QListWidget {{ background-color: {BG_DEEP}; color: {SKY_AQUA}; "
            f"border: 1px solid {TRUE_AZURE}; border-radius: 8px; padding: 8px; "
            f"font-family: Consolas; font-size: 12px; }}"
        )
        content_layout.addWidget(self.proctors_list)
    ...
    def _on_phone_status_changed(self, _name: str):
        """Fired on both phone_connected and phone_disconnected — cheapest
        correct approach is just re-rendering the whole proctor list from
        the thread-safe snapshot, since this list is small (a handful of
        phones per session, not hundreds)."""
        if self.server_thread is None:
            return
        self.proctors_list.clear()
        for phone in self.server_thread.get_connected_phones_snapshot():
            dot = "🟢" if phone["status"] == "connected" else "🔴"
            self.proctors_list.addItem(
                f"{dot} {phone['name']}  ·  {phone['scan_count']} scans  ·  last seen {phone['last_seen'][11:19]}"
            )

    def _on_score_saved(self, student_id: str, total_score: float):
        """Placeholder hook for a future live 'scans this session' stat
        card — logging already happens via log_signal, this is just here
        so SessionManagerPage doesn't need another round of wiring later
        if/when that stat card gets added."""
        pass

    def stop_server_and_return(self):
        """Stops the WebSocket server cleanly and goes back to the Group Hub."""
        if self.server_thread is not None and self.server_thread.isRunning():
            self.server_thread.stop()   # clean asyncio shutdown, not terminate()
