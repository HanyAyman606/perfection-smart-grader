...
DEFAULT_SESSION_PASSWORD = "12345678"
...
class ProjectManager:
    ...
    def get_session_password(self) -> str:
        """Per-workspace phone-session password (separate from the admin
        login password in auth_manager.py). Proctors type this into the
        Flutter app's Connection Screen. Defaults to '12345678' for
        projects that haven't set a custom one yet."""
        config = self.load_config()
        return config.get("session_password", DEFAULT_SESSION_PASSWORD)

    def set_session_password(self, new_password: str):
        self._update_config(session_password=new_password)
